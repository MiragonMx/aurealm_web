...
def load_yaml_mapping(yaml_file):
    ...
    organisations_data = data.get("organisations", {})
    ...
    if isinstance(organisations_data, dict):
        for entry, details in data["organisations"].items():
            slug = details["slug"]
            for word in details["words"]:
                mapping[word] = "organisations/" + slug
